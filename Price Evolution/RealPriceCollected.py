...
constraint = [yearNames, constraints_CO2, constraints_CO2,constraints_CO2]
...
#%% Quantiles data collection - Elec sector coupled (ELEC PRICE)
...
#%% Quantiles data collection - Elec sector coupled (HEAT PRICE)
...
meanPriceHeatWC = []
meanPriceHeatWT = []
quantileMeanPriceHeatC = []
quantileMinPriceHeatC = []
quantileMeanPriceHeatT = []
quantileMinPriceHeatT = []
quantileMeanPriceHeatWC = []
quantileMeanPriceHeatWT = []
...
for file in filename_ElecHeatV2G50:
    ...
    # Country weight
    sizeWeight = loadHeat.sum() / loadHeat.sum().sum()
    ...
    # Prices for Heating for each country (restricted to 1000 €/MWh)
    priceHeat = FilterPrice(priceHeat,465)
    ...
    # weighted average
    meanPriceWC = np.average(np.average(priceHeat,axis=0), weights=sizeWeight)
    meanPriceWT = np.average(np.average(priceHeat,axis=1,weights=sizeWeight))
    
    # append min, max and mean to matrix
    meanPriceHeat.append(meanPrice)
    meanPriceHeatWC.append(meanPriceWC)
    meanPriceHeatWT.append(meanPriceWT)
    
    
    # ----------------------- NP Quantile (Heat) --------------------#
    # --- Elec ---
    # Mean price for flat country
    quantileMeanPriceC = np.quantile(priceHeat.mean(),[0.05,0.25,0.75,0.95])
    
    # Mean price for flat time
    quantileMeanPriceT = np.quantile(priceHeat.mean(axis=1),[0.05,0.25,0.75,0.95])  
    
    # Mean price for weight country
    quantileMeanPriceWC = np.quantile(np.average(priceHeat,axis=0),[0.05,0.25,0.75,0.95])  
    
    # Mean price for weight time
    quantileMeanPriceWT = np.quantile(np.average(priceHeat,axis=1,weights=sizeWeight),[0.05,0.25,0.75,0.95]) 
    
    # append min and mean to matrix (flat country)
    quantileMeanPriceHeatC.append(quantileMeanPriceC)
    
    # append min and mean to matrix (flat time)
    quantileMeanPriceHeatT.append(quantileMeanPriceT)
    
    # append min and mean to matrix (weight country)
    quantileMeanPriceHeatWC.append(quantileMeanPriceWC)
    
    # append min and mean to matrix (weight time)
    quantileMeanPriceHeatWT.append(quantileMeanPriceWT)

# Save values
meanPrices.append(meanPriceHeat) # flat mean prices
meanPricesWC.append(meanPriceHeatWC)  # weighted agross time mean price
meanPricesWT.append(meanPriceHeatWT)  # weighted agross country mean price
quantileMeanPricesC.append(quantileMeanPriceHeatC)  # flat quantiles across countries
quantileMeanPricesT.append(quantileMeanPriceHeatT)  # flat quantiles across time
quantileMeanPricesWC.append(quantileMeanPriceHeatWC)  # weighted quantiles agross countries
quantileMeanPricesWT.append(quantileMeanPriceHeatWT)  # weighted quantiles agross time
