...
#%% Setup Variables
coherenceMismatchC2_1 = []  # Coherence between Electricity and Heating mismatch
coherenceMismatchC2_2 = []  # Coherence between Electricity and Transport mismatch
coherenceMismatchC2_3 = []  # Coherence between Heating and Transport mismatch
coherencePriceC2_1 = []     # Coherence between Electricity and Heating Price
coherencePriceC2_2 = []     # Coherence between Electricity and Transport Price
coherencePriceC2_3 = []     # Coherence between Heating and Transport Price
coherenceElecC2 = []        # Coherence between Electricity mismatch and price
coherenceHeatC2 = []        # Coherence between Heating mismatch and price
coherenceTransC2 = []       # Coherence between Transport mismatch and price
...
for file in filename_CO2:
    ...
    # Append
    coherenceMismatchC1_1.append(c2Mismatch1) 
    coherenceMismatchC1_2.append(c2Mismatch2)
    coherenceMismatchC1_3.append(c2Mismatch3)
    coherencePriceC1_1.append(c2Price1)
    coherencePriceC1_2.append(c2Price2)
    coherencePriceC1_3.append(c2Price3)
    coherenceElecC1.append(c2Elec)
    coherenceHeatC1.append(c2Heat)
    coherenceTransC1.append(c2Trans)
...
for j in range(len(coherenceElecC2)):
    coherenceC2[0,j] = round(np.diagonal(coherencePriceC2_1[j]).sum(),3)
    coherenceC2[1,j] = round(np.diagonal(coherenceElecC2[j]).sum(),3)
    coherenceC2[2,j] = round(np.diagonal(coherenceHeatC2[j]).sum(),3)

# Plot figure
fig = plt.figure(figsize=(10,3), dpi=300)
plt.title("Summed Weighted Eigenvalue Coherence",fontsize=16)

plt.xticks(np.arange(len(constraints)), constraints, fontsize=12)
plt.yticks(ticks=np.linspace(0,1,6), fontsize=12)
plt.grid(alpha=0.3)
plt.plot(coherenceC2[0], marker='o', markersize=5, label = "Electricity mismatch & price coherence")
plt.plot(coherenceC2[1], marker='o', markersize=5, label = "Heating mismatch & price coherence")
plt.plot(coherenceC2[2], marker='o', markersize=5, label = "Electricity & heating price coherence")
plt.ylabel("Coherence [%]", fontsize=14)
plt.ylim([0,1])
plt.legend(loc="lower left", fontsize=12)

# save coherence figure
saveTitle = "Coherence method 2 summed"
