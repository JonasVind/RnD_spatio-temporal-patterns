...
mismatchHeat = mismatchHeat.drop(index=summer)
...
priceHeat = priceHeat.drop(index=summer)
...
for i in range(2):
    ...
    # x-axis 
    axs[i].set_xticks(np.array([0,31,59])+14) # y-axis label (at 14th in the month)
    axs[i].set_xticklabels(["Jan","Feb","Dec"],rotation=-90) # y-axis label
...
#%% Plot and save
path = figurePath
title = "Comparison between Heating Prices and Mismatch (winter)"
